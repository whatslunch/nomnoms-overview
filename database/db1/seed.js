...
pg.connect('postgres://localhost:5432/nomnoms', (err, client, done) => {
  client.query('DROP TABLE IF EXISTS restaurants')
    .on('end', () => {
      ...
      const pgslStream = client.query(copyFrom('COPY restaurants FROM STDIN CSV HEADER'));
...
pg.connect('postgres://localhost:5432/nomnoms', (err, client, done) => {
  client.query('DROP TABLE IF EXISTS images')
    .on('end', () => {
      client.query('CREATE TABLE IF NOT EXISTS images(id SERIAL PRIMARY KEY, src VARCHAR(100), restaurant_id INTEGER)');
      const pgslStream = client.query(copyFrom('COPY images FROM STDIN CSV HEADER'));
