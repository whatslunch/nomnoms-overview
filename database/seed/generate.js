...
const genRestaurantSeed = (n, cb) => {
  ...
  const write = () => {
    while (i < n) {
      ...
      let data = {
        name: `${[pref, base, suff, inst].join(' ').split(',').join('').split('  ').join(' ').trim()}`,
        lat: getRandomBetween(37.720151, 37.785006, 6),
        lng: getRandomBetween(-122.397980, -122.498002, 6),
        address: `${faker.fake('{{address.streetAddress}}, San Francisco, CA 941' + getRandomBetween(40, 10, 0)).split(',').join('')}`,
...
genRestaurantSeed(20000000, () => {
  console.log('all done');
});

genImageSeed(20000000, () => {
  console.log('all done');
});
